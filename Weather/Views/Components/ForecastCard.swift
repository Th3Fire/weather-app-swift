...
struct ForecastCard: View {
    ...
    var forecastPreiod: ForecastPeriod
    
    var isActive: Bool {
        if forecastPreiod == ForecastPeriod.hourly {
    ...
    var body: some View {
        ZStack {
            ...
            // MARK: Content
            VStack(spacing: 16) {
                Text(forecast.date, format: forecastPreiod == ForecastPeriod.hourly ? .dateTime.hour() : .dateTime.weekday())
...
struct ForecastCard_Previews: PreviewProvider {
    static var previews: some View {
        ForecastCard(forecast: Forecast.hourly[0], forecastPreiod: .hourly)
    }
}
